...
import mangaActions from '../store/actions/mangaud'
import Swal from 'sweetalert2'
import { Link as Anchor, useNavigate } from 'react-router-dom'
import Editmangas from './Editmanga'

const { categoriesRead } = categoriesActions
const { mangaDelete, mangaRead, mangaUpdate } = mangaActions
...
export default function MangasCreate () {
  ...
  function checkedId () {
    const categories = Object.values(category_id.current)
    const values = []
          ...
          <div>
            ...
            <div className=' h-fit bg-slate-200 flex justify-center'>
              <div className='min-h-screen rounded-[206px_206px_2px_27px/64px_64px_0px_1px;] w-full mt-[-5rem] lg:mt-[-3rem] lg:bg-white lg:rounded-2xl lg:w-11/12 xl:mt-[-3rem] mb-8 bg-slate-200 xl:bg-white xl:rounded-2xl xl:w-11/12 xl:m-[-2rem] 2xl:bg-white 2xl:rounded-2xl 2xl:w-11/12 2xl:m-[-5rem]'>
                ...
                <Anchor to='/mangas-form' className='flex justify-center items-center flex-col-reverse'>Add manga
                    ...
                    <div key={manga._id} className='bg-white shadow-xl mt-4 rounded-2xl ml-4 mr-4'>
                      <div className='flex h-48 lg:h-56 xl:h-48 2xl:h-56'>
                        <div className='flex items-center'>
                          <div className='border-l-8 h-2/3' style={{ borderColor: manga?.category_id.color }} />
                        </div>
                        <div className='flex w-3/5 flex-col justify-between'>
                          ...
                          <div>
                            ...
                            <p className='flex items-center  font-bold ml-4' />
                          ...
                          <div className='flex justify-around'>
                            ...
                            <button onClick={() => alertDelete(() => dispatch(mangaDelete({ id: manga?._id })))} className=' w-5/12 my-2  rounded-lg text-[#EF8481] bg-[#FFE0DF] '>Delete</button>
                          </div>
                        </div>
